            ...
            articles: [
                {
                    ...
                    number: 144,
                    title: "المادة 144",
                    content: `لا يجوز خلال دورة انعقاد المجلس ملاحقة العضو جزائياً أو اتخاذ إجراءات جزائية أو إمارية بحقه أو إلقاء القبض عليه أو توقيفه إلا بإذن المجلس، باستثناء حالة الجرم الجنائي المشهود، وفي حالة القبض عليه بهذه الصورة يجب إعـام المجلس بذلك فوراً.`
                },
                {
                    id: "article145",
                    number: 145,
...
class NavigationSystem {
        ...
        return `
            ...
            <div class="article-content">
                ...
                <div class="article-navigation">
                    <button class="nav-btn prev-btn" onclick="navigation.navigateToPreviousArticle('${article.id}')">
                        ← المادة السابقة
                    </button>
                    <button class="nav-btn next-btn" onclick="navigation.navigateToNextArticle('${article.id}')">
                        المادة التالية →
                    </button>
                </div>
            </div>
    ...
    navigateToPreviousArticle(currentArticleId) {
        const allArticles = this.getAllArticles();
        const currentIndex = allArticles.findIndex(article => article.id === currentArticleId);
        
        if (currentIndex > 0) {
            const prevArticle = allArticles[currentIndex - 1];
            this.loadArticle(prevArticle.id);
        }
    }

    navigateToNextArticle(currentArticleId) {
        const allArticles = this.getAllArticles();
        const currentIndex = allArticles.findIndex(article => article.id === currentArticleId);
        
        if (currentIndex < allArticles.length - 1) {
            const nextArticle = allArticles[currentIndex + 1];
            this.loadArticle(nextArticle.id);
        }
    }
...
class LegalMindApp {
    ...
    showNotification(message, type = 'info') {
        ...
        notification.style.cssText = `
            ...
            background: ${type === 'error' ? '#dc2626' : type === 'success' ? '#059669' : '#1e3a8a'};
            color: white;
            padding: 12px 24px;
            border-radius: 8px;
